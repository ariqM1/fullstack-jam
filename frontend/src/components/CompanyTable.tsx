...
import { Button, Box, Select, MenuItem, FormControl, InputLabel, LinearProgress, Typography, Alert } from "@mui/material";
...
const CompanyTable = (props: { 
  ...
  const [selectedCompanies, setSelectedCompanies] = useState<GridSelectionModel>([]);
  const [targetCollectionId, setTargetCollectionId] = useState<string>('');
  const [operations, setOperations] = useState<Map<string, IOperationStatus>>(new Map());
      ...
      <Box sx={{ mb: 2, p: 2, border: '1px solid #333', borderRadius: 1 }}>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', mb: 2 }}>
          ...
          <Button
            ...
            disabled={!targetCollectionId || selectedCompanies.length === 0 || loading}
            size="small"
          >
          ...
          <Box sx={{ mb: 2 }}>
            <Typography variant="body2" sx={{ mb: 1 }}>
              {activeOperation.status === 'in_progress' 
                ? `Processing... ${activeOperation.progress || 0}/${activeOperation.total || 0} companies`
            ...
            {activeOperation.status === 'in_progress' && (
              <LinearProgress 
                variant="determinate" 
                value={activeOperation.total ? (activeOperation.progress || 0) / activeOperation.total * 100 : 0}
              />
            )}
          </Box>
        )}
        ...
        {/* Error Display */}
        {Array.from(operations.values()).some(op => op.status === 'failed') && (
          <Alert severity="error" sx={{ mb: 2 }}>
            Operation failed: {Array.from(operations.values()).find(op => op.status === 'failed')?.error_message}
          </Alert>
        )}
      </Box>
